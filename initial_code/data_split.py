...
def get_loaders(
    ...
    """
    # First sample test sequences (capped at 1000)
    test_size = min(1000, int(num_samples * (1 - train_ratio)))
    test_sequences = sample_sequences(energy_params, test_size)
    
    # Then sample training sequences
    train_sequences = sample_sequences(energy_params, num_samples)
    
    # Remove any training sequences that appear in test set
    test_sequences_set = set(tuple(seq) for seq in test_sequences)
    train_sequences = [seq for seq in train_sequences if tuple(seq) not in test_sequences_set]
